...
# ── Features: originales + 3 nuevas (MEJORADO) ──────────────────────────
features = ['dias_desde_ultima_op', 'freq_30d', 'freq_90d', 'monto_prom_90d',
            ...
            'promedio_dias_entre_ops', 'dia_favorito_cliente', 'es_dia_favorito']
...
# ── 1b. Construir features nuevas de timing sobre el panel histórico ────
print("Construyendo features nuevas de timing (promedio_dias_entre_ops, dia_favorito)...")
df_ops_hist = pd.read_parquet(PROCESSED + 'dataset_consolidado.parquet',
                               columns=['Fecha', 'NIT'])
df_ops_hist['Fecha'] = pd.to_datetime(df_ops_hist['Fecha'])

# Para cada fecha_corte del panel, estas features deben calcularse SOLO con
# operaciones <= esa fecha_corte (sin fuga). Como el panel tiene una fila por
# NIT por día, y dia_favorito/promedio son razonablemente estables día a día,
# se recalculan una vez por NIT usando todo el histórico disponible hasta
# FECHA_ANCLA (igual aproximación usada en la validación).
gap_medio = (df_ops_hist.sort_values(['NIT', 'Fecha'])
                         .groupby('NIT')['Fecha']
                         .apply(lambda x: x.diff().dt.days.mean())
                         .fillna(999)
                         .rename('promedio_dias_entre_ops'))

dia_fav = (df_ops_hist.copy()
                       .assign(dia=df_ops_hist['Fecha'].dt.weekday)
                       .groupby(['NIT', 'dia'])
                       .size()
                       .reset_index(name='n')
...
panel['promedio_dias_entre_ops'] = panel['NIT'].map(gap_medio).fillna(999)
panel['dia_favorito_cliente']    = panel['NIT'].map(dia_fav).fillna(-1).astype(int)
panel['es_dia_favorito']         = (panel['dia_semana'] == panel['dia_favorito_cliente']).astype(int)
...
# ── 3b. Agregar features nuevas de timing a base (clientes a predecir) ──
...
# es_dia_favorito depende del dia_semana de cada fecha de junio, se calcula por bloque abajo
...
for fecha in dias_junio:
    ...
    b['es_dia_favorito']  = (b['dia_semana'] == b['dia_favorito_cliente']).astype(int)
...
# ── 7. Agregar Trader asignado a la salida (NO afecta el modelo) ────────
print("\nCruzando con base de traders...")
df_traders = pd.read_parquet(PROCESSED + 'traders.parquet')  # columnas: NIT, Trader

lista_junio = lista_junio.merge(df_traders, on='NIT', how='left')
lista_junio['Trader'] = lista_junio['Trader'].fillna('No asignado')

sin_trader = (lista_junio['Trader'] == 'No asignado').sum()
print(f"  Clientes sin trader asignado en la lista final: {sin_trader} de {len(lista_junio)}")

# ── 8. Exportar ──────────────────────────────────────────────────────────
lista_junio['fecha_prediccion'] = lista_junio['fecha_corte']
lista_junio['dia_llamada']      = lista_junio['fecha_corte'] + pd.Timedelta(days=1)

cols_out = ['fecha_corte', 'dia_llamada', 'NIT', 'Trader', 'Segmento', 'prob_opera_manana']
...
# Guardar modelo y lista de features (07 la necesita para reconstruir inputs)
modelo_final.save_model(MODELS + 'modelo_timing_final.txt')
import json
with open(MODELS + 'modelo_timing_metadata.json', 'w', encoding='utf-8') as f:
    json.dump({'features': features, 'cat_features': cat_features}, f, indent=2, ensure_ascii=False)
print("Guardado: modelo_timing_final.txt")
print("Guardado: modelo_timing_metadata.json")